...
server = function(input, output, session) {
  ...
  observe({
    if(input$action32 > 0){
      print('32')
      session$sendCustomMessage("myCallbackHandler", "32")
    }
  })
  ...
  observe({
    if(input$action33 > 0){
      print('33')
      session$sendCustomMessage("myCallbackHandler", "33")
    }
  })
  ...
  observe({
    if(input$action31 > 0){
      print('31')
      session$sendCustomMessage("myCallbackHandler", "31")
    }
  })
  
  ############################################################
  #     Leitura do arquivo CSV E XLSX                             #
  ############################################################
  ...
  output$menu <- renderPrint({
    ...
    if ((input$deli!="faixas")&&(input$deli!="fat2addbc")&&(input$deli!="fat2addic")&&(input$deli!="fat2dbc")&&(input$deli!="fat2dic")&&(input$deli!="fat3addbc")&&(input$deli!="fat3addic")&&(input$deli!="fat3dbc")&&(input$deli!="fat3dic")&&(input$deli!="psub2dbc")&&(input$deli!="psub2dic") && (input$list_easy != "Todas")) {
  ...
  output$menu5<- renderPrint({
    ...
    selectInput("box1", label = "Selecione a 1 variável do Boxplot do DBC [TRATAMENTO]",
                choices = names(dataset),
                selected = 1)
  })
  ...
  output$menu6<- renderPrint({
    ...
    selectInput("box2", label = "Selecione a 2 variável do Boxplot do DBC [VARIAVEL DE INTERESSE]",
                choices = names(dataset),
                selected = 3)
  })
  ...
  output$menu9<- renderPrint({
    ...
    if((input$deli!="faixas")&&(input$deli!="fat2addbc")&&(input$deli!="fat2addic")&&(input$deli!="fat2dbc")&&(input$deli!="fat2dic")&&(input$deli!="fat3addbc")&&(input$deli!="fat3addic")&&(input$deli!="fat3dbc")&&(input$deli!="fat3dic")&&(input$deli!="psub2dbc")&&(input$deli!="psub2dic")){
      ...
      radioButtons("quali", "A variável Tratamento é:", inline = TRUE,
  ...
  output$transformacao<- renderPrint({
    
    radioButtons("opcao_transformacao", "Selecione o teste de Transformação de dados",
                 choices = c('Sem tranformação' = "sem transf",
                             'Raiz(x)' = "raizX",
                             'Raiz(x + 0.5)' = "raizX05",
                             'Log(X)' = "logX",
                             'Log(X + 0.5)' = "logX05"),
  ...
  output$conjunto <- renderPrint({
    ...
    actionButton(input$conj, label = "Avancar >", class="btn btn-success", width="100")
  ...
  output$avancar <- renderPrint({
    ...
    actionButton(input$deli, label = "Avancar >", class="btn btn-sucess", width="100")
  ...
  aplicar_transformacao = function(dados_database){
    dados_transformados = dados_database
    
    if(input$opcao_transformacao == "sem transf"){
      return (dados_transformados)
    }
    if(input$opcao_transformacao == "raizX"){
          dados_transformados <- sqrt(dados_database)
      return (dados_transformados)
    }
    if(input$opcao_transformacao == "raizX05"){
      dados_transformados <- sqrt(dados_database + 0.5)
      return (dados_transformados)
    }
    if(input$opcao_transformacao == "logX"){
      dados_transformados <- log(dados_database)
      return (dados_transformados)
    }
    if(input$opcao_transformacao == "logX05"){
      dados_transformados <- log(dados_database + 0.5)
      return (dados_transformados)
    }
  }
  ...
  output$anovatable <- renderPrint({
    ...
    variavel_Tratada <- aplicar_transformacao(respos)

    dic(tratas, variavel_Tratada, quali=input$quali, mcomp=input$mcomp, nl=input$nl1, hvar=input$hvar, sigT=sigT, sigF=sigF)
  ...
  #######################################################
  # Tabela ANOVA DIC  EASYANOVA                         #
  #######################################################
  output$ANOVA_DIC_TABLE <- renderPrint({
    ...
    cat("\n")
    base_individual <- data.frame(tratas, respos)
    ############# Listar variáveis ou não
    lista <- function(){
      if (input$list_easy == "Uma"){
        return (FALSE)
      }else{
        return (TRUE)
      }
    ...
    cat(paste("Teste a: ", as.character(input$sigF_easy_dic), "% de significância"))
    cat("\n\n")
    if (input$list_easy == "Uma"){
      dados <- base_individual
      resposta1=names(respo)
      cat(resposta1)
      cat("\n")
    ...
    dic_easy <- ea1(dados, alpha = input$sigF_easy_dic / 100, list = lista())
    dic_easy
  })
  ...
  output$anovatable8 <- renderPrint({
    ...
    fat3 <- unlist(fatoo)
    respadi<-na.omit(dataset[input$respad])
    respAd <- unlist(respadi)
    print(respAd)
  ...
  output$plotar1 <- renderPlotly({
    ...
    p <- plot_ly(dataset, x = ~tratamento, y= ~resposta, type = "box", color = I("gray50"), marker = list(color = "black"), legend = F, showlegend = F, jitter = FALSE,
                 pointpos = -0.1, xlab = "Tratamentos", ylab= "Fungicidas")  %>% 
  ...
  output$teste_escolhido <- renderPrint(input$mcomp)  
  output$barras <- renderPlotly({
    #captura o teste em analise
    
    teste_escolhido <- input$mcomp
    ...
    df <- data.frame(tratamento, resposta)
    df$tratamento <- as.factor(df$tratamento)
    model <-  aov(resposta ~ tratamento, data = df)
    
    rs <- df.residual(model)
    MSerror <- deviance(model) / rs 
    
    if (teste_escolhido == "tukey"){ 
      t <- with(dataset, HSD.test(resposta, tratamento, rs, MSerror, group = TRUE, console = TRUE))
      cld <- t$groups$groups
    }
    if (teste_escolhido == "lsd"){ 
      t <- with(dataset, LSD.test(resposta, tratamento, rs, MSerror, group = TRUE, console = TRUE))
      cld <- t$groups$groups
      
    }
    if (teste_escolhido == "duncan"){ 
      t <- with(dataset, duncan.test(resposta, tratamento, rs, MSerror, group = TRUE, console = TRUE))
      cld <- t$groups$groups
    }
    if (teste_escolhido == "snk"){ 
      t <- with(dataset, SNK.test(resposta, tratamento, rs, MSerror, group = TRUE, console = TRUE))
      
    }
    if (teste_escolhido == "ccf"){
      showModal(modalDialog("Gráfico para este Teste ainda não implementado! Altere o Teste ou aguarde! "))
    }
    if (teste_escolhido == "ccboot"){
      ShowModal(modalDialog("Gráfico para este Teste ainda não implementado! Altere o teste ou aguarde! "))
    }
    
    if (teste_escolhido == "sk"){ 
      t <- ea1(df, design = 1)
    }
    ...
    dt <- group_by(df, tratamento) %>%
      summarise(w=mean(resposta), sd = sd(resposta)) %>%
      arrange(desc(w))
    
    #Conferir
    if(teste_escolhido == "sk"){
      cld <- t$Means$scott_knott
    }
    else{
      cld <- t$groups$groups
    }
    ...
    m <- {
      ggplot(dt, aes(tratamento, w)) + 
        geom_bar(stat = "identity", aes(fill = w), show.legend = FALSE) +
        geom_errorbar(aes(ymin = w-sd, ymax=w+sd), width = 0.2) +
        xlab(labelX) +
        ylab(labelY) + 
        geom_text(aes(label = cld, y = w+sd+(0.1*(w+sd))), vjust = 0) +
        theme_few() 
    }
    m
  })
  
  # Grafico de barras invertido
  
  output$barrasinvertida <- renderPlotly({
    #captura o teste em analise
  ...
  theplot <- reactive({
    ...
    dadoscorrel<- data.frame(col2)
    
    cor <- correlation(dadoscorrel) 
    plot(cor)
  })
  ...
  # output$btnDownCorrelMapaDIC <- downloadHandler(
  #  filename = "correlacao.png",
  ...
  output$pca <- renderPlot({
    ...
    pca <- fviz_pca_var(res.pca,
                        col.var = "contrib", # Contribuição por cores
  ...
  output$correlDICsig <- renderPlot({
    dados <- whichdataset()
    dados <- data.frame(dados)
    dados[1] <- NULL
    dados[1] <- NULL 
    dadoscorrelograma<- data.frame(dados)
    
    M <- cor(dadoscorrelograma)
    ...
    p.mat <- cor.mtest(dadoscorrelograma)
  ...
  output$correlDICNumeric <- renderPlot({
    dados <- whichdataset()
    dados <- data.frame(dados)
    dados[1] <- NULL
    dados[1] <- NULL 
  ...
  output$plotar <- renderPlotly({
    ...
    p <- plot_ly(dataset, x = campo11, y= campo21, type = "box", boxpoints = "all", color = I("gray50"), marker = list(color = "black"), legend = F, jitter = FALSE,
                 pointpos = -0.1)
    p
  ...
  output$teste_escolhido1 <- renderPrint(input$mcomp1)  
  
  ###############################################
  ###############################################
  ## BARRAS DBC BLACK
  
  output$barrasDBCblack <- renderPlotly({
    #captura o teste em analise
    
    teste_escolhido1 <- input$mcomp1
    ...
    df <- data.frame(tratamento, blocos, resposta)
    df$tratamento <- as.factor(df$tratamento)
    df$blocos <- as.factor(df$blocos)
    model <-  aov(resposta ~ tratamento + blocos, data = df)
    
    rs <- df.residual(model)
    MSerror <- deviance(model) / rs 
    
    if (teste_escolhido1 == "tukey"){ 
      t <- with(model, HSD.test(resposta, tratamento, rs, MSerror, group = TRUE, console = TRUE))
      cld <- t$groups$groups
    }
    if (teste_escolhido1 == "lsd"){ 
      t <- with(model, LSD.test(resposta, tratamento, rs, MSerror, group = TRUE, console = TRUE))
      cld <- t$groups$groups
      
    }
    if (teste_escolhido1 == "duncan"){ 
      t <- with(model, duncan.test(resposta, tratamento, rs, MSerror, group = TRUE, console = TRUE))
      cld <- t$groups$groups
    }
    if (teste_escolhido1 == "snk"){ 
      t <- with(model, SNK.test(resposta, tratamento, rs, MSerror, group = TRUE, console = TRUE))
      
    }
    if (teste_escolhido1 == "ccf"){
      showModal(modalDialog("Gráfico para este Teste ainda não implementado! Altere o Teste ou aguarde! "))
    }
    if (teste_escolhido1 == "ccboot"){
      ShowModal(modalDialog("Gráfico para este Teste ainda não implementado! Altere o teste ou aguarde! "))
    }
    ...
    #mutate(trt = reorder(trt, -volume, mean)) %>%
    labelX <- input$labelXDBC
    labelY <- input$labelYDBC
    
    m <- {
      ggplot(dt, aes(tratamento, w)) + 
        geom_col(alpha = 0.8, color = "black", aes(fill = w)) +
        geom_text(aes(label = cld, y = (w - (0.1 * w))), size = 7, color = "white") +
        labs(x = labelX, y = labelY) +
        theme_classic(16)
    }
    
    if (teste_escolhido1 == "sk"){ 
      
      df <- data.frame(factor(tratamento), factor(tratamento), resposta) 
      anova <-  aov(resposta ~ factor.tratamento., data = df)
      sk <- with(df, ea1(df[,-2]), design = 1, list = T) 
      
      
      cld <- sk$Means$scott_knott
      
      dt <- group_by(df, factor.tratamento.) %>%
        summarise(w=mean(resposta), sd = sd(resposta)) %>%
        arrange(desc(w))
      
      max <- max(as.numeric(dt$factor.tratamento.))
      #mutate(trt = reorder(trt, -volume, mean)) %>%
      m <- {
        ggplot(dt, aes(tratamento, w)) + 
          geom_col(alpha = 0.8, color = "black", aes(fill = w)) +
  ...
  output$boxplotDBC <- renderPlotly({
    #captura o teste em analise
    ...
    m <- {
      ggplot(df, aes(tratamento, resposta)) + 
        geom_boxplot(fill = "grey60", alpha = 0.6, outlier.color = "red") +
        labs(x = labelX, y = labelY) +
        theme_classic(16) 
    }
  ...
  output$barrasDBC <- renderPlotly({
    ...
    #Conferir
    if(teste_escolhido1 == "sk"){
      cldDBC <- t$Means$scott_knott
    }
    else{
      cldDBC <- t$groups$groups
    }
    #mutate(trt = reorder(trt, -volume, mean)) %>%
    ...
    m <- {
      ggplot(dt, aes(tratamento, w)) + 
        ...
        xlab(labelX) +
        ylab(labelY) + 
        geom_text(aes(label = cldDBC, y = w+sd+(0.1*(w+sd))), vjust = 0) +
        theme_few() 
    }
    m
  })
  ...
  output$barrasinvertidaDBC <- renderPlotly({
    #captura o teste em analise
    ...
    df <- data.frame(tratamento, resposta, blocos)
    df$tratamento <- as.factor(df$tratamento)
    df$blocos <- as.factor(df$blocos)
    model <-  aov(resposta ~ tratamento + blocos, data = df)
  ...
  output$correlDBCsig <- renderPlot({
    dados <- whichdataset()
    dados <- data.frame(dados)
    dados[1] <- NULL
    dados[1] <- NULL 
  ...
  output$correlDBCNumeric <- renderPlot({
    dados <- whichdataset()
    dados <- data.frame(dados)
    dados[1] <- NULL
    dados[1] <- NULL 
  ...
  output$resid_plot8 <- renderPlot({
    ...
    respadi1<-dataset[input$respad]
    #respadi1 <- na.omit(respadi1)
    adicional <- na.omit(respadi1)
    # adicional = respadi1[!is.na(respadi1)]
    respadi<-dataset[input$respad]
    respAd <- unlist(adicional)
    print(adicional)
  ...
  if(input$metodologia == 'expDes'){
    ...
    if(input$quali == 'FALSE') {
      ...
      tabsetPanel(
        ...
        tabPanel("Análise de Resíduo",          plotOutput("resid_plot")),
        tabPanel("Gráficos",             
                 verbatimTextOutput("teste_escolhido"),
                 plotlyOutput("barras"),
                 plotlyOutput("barrasinvertida"),
                 plotlyOutput("plotar1")),
        tabPanel("Multivariada",
                 textOutput("mensagemMultivariada"),
                 textOutput("correlMapaDICText"),
                 plotOutput("correlacao"),
                 textOutput("pcaDICText"),
                 plotOutput("pca"),
                 textOutput("correlDICNumericText"),
                 plotOutput("correlDICNumeric"),
                 textOutput("correlDICSigText"),
                 plotOutput("correlDICsig"))
      )
    }
    
  }else{
    
  if (input$metodologia == 'easyanova' & input$deli_easyanova == 'dic_easyanova'){
      tabsetPanel(
        tabPanel("ANOVA - DIC",       verbatimTextOutput("ANOVA_DIC_TABLE")),
      )
    }
  }  
  ...
  if(input$metodologia == 'expDes'){
    ...
    if(input$quali == 'FALSE') {
      ...
      tabsetPanel(
        ...
        tabPanel("ANOVA",       verbatimTextOutput("anovatable1")),
        tabPanel("Análise de Resíduo",          plotOutput("resid_plot1")),
        tabPanel("Gráficos",             
                 verbatimTextOutput("teste_escolhido1"),
                 textOutput("graficosDBCText"),
                 plotlyOutput("barrasDBCblack"),
                 plotlyOutput("boxplotDBC"),
                 plotlyOutput("barrasDBC"),
                 plotlyOutput("barrasinvertidaDBC"),
                 plotlyOutput("plotar")),
        tabPanel("Multivariada",
                 textOutput("correlMapaDBCText"),
                 plotOutput("correlacao1"),
                 textOutput("pcaDBCText"),
                 plotOutput("pca1"),
                 textOutput("correlDBCNumericText"),
                 plotOutput("correlDBCNumeric"),
                 textOutput("correlDBCSigText"),
                 plotOutput("correlDBCsig"))
       )
     }
   }
    
    if (input$metodologia == 'easyanova'){
      tabsetPanel(
           tabPanel("ANOVA_DIC_EASY",       verbatimTextOutput("ANOVA_DIC_TABLE")),
           cat("Entrou aqui 2 ")
      )
    }
  })
  ...
  #######################################################
  # DBC
  output$correlMapaDBCText <- renderText(" > Mapa de correlação de variáveis \n", )
  output$pcaDBCText <- renderText(" > Análise de componentes Principais - PCA \n")
  output$correlDBCSigText <- renderText (" > Correlograma - Interação significativa à p. <0.05 \n")
  output$correlDBCNumericText <- renderText(" > Correlograma com coeficientes de interação \n")
  
  # Graficos medias
  output$graficosDBCText <- renderText(" A seguir uma opção de gráficos pré-prontos (Habilitado para DBC e DIC Comum) - para salvar vá ao menu opções no canto superior direito do gráfico. \n", )
  
  #DIC 
  output$mensagemMultivariada <- renderText("Para salvar: Clique com botão secundário > Salvar imagem como: \n", )
  output$correlMapaDICText <- renderText("\n > Mapa de correlação de variáveis - [Gráfico 1/4] \n", )
  output$pcaDICText <- renderText(" > Análise de componentes Principais - PCA [Gráfico 2/4] \n")
  output$correlDICSigText <- renderText (" > Correlograma - Interação significativa à p. <0.05 [Gráfico 3/4] \n")
  output$correlDICNumericText <- renderText(" > Correlograma com coeficientes de interação [Gráfico 3/4] \n")
